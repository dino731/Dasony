...
// -----------------------------검색창-----------------------------------
    ...
    const [chatData, setChatDate] = useState ([
        {id : 1, chatname : '안 고독한 박명수', username : '말미잘', people : 20},
...
// -----------------------------새 채팅방(채팅방 이름)-----------------------------------    
    ...
    const handleChatRoomCreate = (chatRoomName) => {
       ...
       navigate(`/chat/${newChatRoom.chatname}`);
                        ...
                        <table id="tchattable">
                            <tbody style={{height: '100%'}}>
                                ...
                                <tr key={chat.id}>
                                    ...
                                    <td width="410">{chat.chatname}</td>
                                    <td width="260">{chat.username}</td>
                                    <td width="150">{chat.people}명</td>
                                    <td width="150">
                                    <Link to={`/chats/${chat.chatname}`}><button>참여하기</button></Link>
                                    </td>
                                </tr>
                                ))}
                            </tbody>
